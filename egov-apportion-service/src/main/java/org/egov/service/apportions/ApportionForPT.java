...
import static org.egov.util.ApportionConstants.PT;

@Service
public class ApportionForPT implements ApportionV2 {
	...
	@Autowired
	public ApportionForPT(TaxHeadMasterService taxHeadMasterService, ApportionConfig config) {
	...
	public List<TaxDetail> apportionPaidAmount(ApportionRequestV2 apportionRequestV2, Object masterData) {
		System.out.println("apportion fot PT");
		...
		for (TaxDetail taxDetail : taxDetails) {
			...
			if (!config.getApportionByValueAndOrder())
				...
				taxDetail.getBuckets().sort(Comparator.comparing(Bucket::getPriority));
			...
			for (Bucket bucket : taxDetail.getBuckets()) {
				...
				if (isAmountPositive) {
					if (remainingAmount.equals(BigDecimal.ZERO)) {
						...
						// handling interest and penalty apportion(adjusting based on priority)
						if (ApportionConstants.ADDITIONAL_TAXES.contains(bucket.getTaxHeadCode())) {
		...
		System.out.println("taxdetails::" + new Gson().toJson(taxDetails));
		return taxDetails;
	}

	private BigDecimal getTotalAmountForTaxDetail(TaxDetail taxDetail) {
		BigDecimal totalAmt = BigDecimal.ZERO;
		List<Bucket> bockets = taxDetail.getBuckets();
		for (Bucket bucket : bockets) {
			if (!ApportionConstants.ADDITIONAL_TAXES.contains(bucket.getTaxHeadCode())) {
			BigDecimal amt = bucket.getAmount().subtract(bucket.getAdjustedAmount());
